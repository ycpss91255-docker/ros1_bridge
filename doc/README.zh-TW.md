...
**方式 2：一次性 `--build-arg` override（直接 docker build）**。會繞開
`./build.sh`，不會更新 `.env` / `compose.yaml`。wrapper 端 `./build.sh
...
./build.sh runtime               # 建置精簡的 runtime image
```
...
不想動 symlink 也可以直接 build 時覆蓋。wrapper 端 `./build.sh --build-arg
