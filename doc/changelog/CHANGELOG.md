...
- **Adopted base v0.30.0 `[logging] local_path`** so demo / debug runs leave host-readable log files under `<repo>/logs/<service>.log` ([closes #101](https://github.com/ycpss91255-docker/ros1_bridge/issues/101); ships in [base v0.30.0](https://github.com/ycpss91255-docker/base/releases/tag/v0.30.0) via [base#328](https://github.com/ycpss91255-docker/base/issues/328) / [base PR #356](https://github.com/ycpss91255-docker/base/pull/356)). Three pieces wire up together:
  ...
  - `config/docker/setup.conf` gains a `[logging]` section listing all 5 keys (`driver` / `max_size` / `max_file` / `compress` / `local_path = ./logs/`) — section-replace strategy means we must list every key we want to keep from the template default, not just the local_path override.
  - `script/entrypoint.sh` sources `.base/script/docker/_entrypoint_logging.sh` immediately after `set -euo pipefail` (line 2), before the ROS overlay sourcing block. The helper reads `LOG_FILE_PATH` env (injected by setup.sh per service: `devel.log` / `runtime.log` / `builder.log` / `test.log`), truncates the file, then `exec > >(tee -a "$LOG_FILE_PATH") 2>&1` rebinds stdout/stderr so the bridge publisher output lands in both `docker logs <c>` and the host file. No-op when `LOG_FILE_PATH` is unset, so the line is safe regardless of whether `local_path` is configured.
  - `/logs/` auto-appended to `.gitignore` by `setup.sh apply` (no manual gitignore edit needed).
  - Verification: `./build.sh -d` then `cat logs/runtime.log` matches `docker logs ros1_bridge-runtime`.

### Changed
- **Migrated `.github/workflows/main.yaml` to the multi-distro dispatcher reusable workflow** ([base#325 B-1](https://github.com/ycpss91255-docker/base/issues/325), ships in base v0.29.0 with the v0.29.1 hyphen-separator correction). `call-docker-build` no longer carries an inline `strategy.matrix: { ros2_distro: [humble, jazzy] }` + per-event matrix expression; instead it calls `ycpss91255-docker/base/.github/workflows/multi-distro-build-worker.yaml@v0.29.1` with `image_name: ros1_bridge`, `pr_distros: '["humble"]'`, `tag_distros: '["humble", "jazzy"]'`, `distro_input_name: ROS2_DISTRO`. The dispatcher resolves the per-event distro subset internally (PR -> `pr_distros`; tag / main push / workflow_dispatch -> `tag_distros`) so PR-time builds humble only — cuts PR wall-time roughly in half while tag-time still validates humble + jazzy. Per-shard image_name shape `ros1_bridge-${distro}` is preserved by the dispatcher's hyphen separator. `ci-summary` aggregator job kept locally so the branch-protection `ci-summary` required-check name doesn't change; it now mirrors the dispatcher's overall rollup result. `call-release` bumped `@v0.28.2 -> @v0.29.1` while keeping its 1D matrix tag-time shape (the dispatcher is build-only). `test_tools_version` bumped `v0.20.1 -> v0.29.1` to match.

- **Upgraded `.base/` subtree to [v0.28.0](https://github.com/ycpss91255-docker/base/releases/tag/v0.28.0)** (closes [#98](https://github.com/ycpss91255-docker/ros1_bridge/issues/98)). Notable changes downstream sees:
  - **`[logging]` section** on every compose service ([base#310](https://github.com/ycpss91255-docker/base/issues/310)): generated `compose.yaml` now emits `logging: driver: json-file, options: { max-size: "10m", max-file: "3" }` on `devel` / `runtime` / `builder` / `test-tools-stage`. Long-running `./run.sh -t runtime` no longer accumulates an unbounded `/var/lib/docker/containers/<id>-json.log`. No `[logging]` section in `<repo>/config/docker/setup.conf` — section-replace lets the base default kick in. Override locally if you need different limits.
  - **`-v / -vv` flag on all 4 wrappers** ([base#311](https://github.com/ycpss91255-docker/base/issues/311)): `./build.sh -v --no-cache` now shows BuildKit `plain` progress including the test-tools image build (previously silent under `-q >/dev/null`).
  - **`_lib.sh` split into focused sub-libs** under `.base/script/docker/lib/{log,env,conf,compose,config_summary,gitignore}.sh` ([base#284](https://github.com/ycpss91255-docker/base/issues/284)). `_lib.sh` is now an umbrella loader.
  - **`actionlint` gates downstream jobs** in base self-test CI ([base#305](https://github.com/ycpss91255-docker/base/issues/305)) -- affects base PR cycle, not our CI directly.
- **Dockerfile lint stage now COPYs `.base/script/docker/lib/`** to `/lint/lib/` so `_lib.sh`'s `source "${_lib_dir}/lib/log.sh"` chain works during the ShellCheck stage. Without this, all 25 `-h` / `--help` / `--lang` smoke tests fail with `/lint/lib/log.sh: No such file or directory`. The fix follows the v0.28.0 lib-split pattern: bring the sub-libs along wherever the umbrella loader goes. ShellCheck pass extended to `/lint/lib/*.sh` for completeness. **All other downstream repos will need the same Dockerfile fix when they upgrade to v0.28.0** -- worth filing as a base issue so the template's `Dockerfile.example` reflects the new structure.
...
- **Demo publishers replaced with long-lived Python helpers** so the publish rate is actually configurable instead of floored at ~0.6 Hz by per-iteration rospy / rclpy spinup. New helpers `script/demo_pub_ros1.py` (rospy.Publisher) and `script/demo_pub_ros2.py` (rclpy node + Timer) live at `/root/demo/` inside the image. Default rate is now **10 Hz** (was ~0.6 Hz under the bash + `rostopic pub --once` loop pattern from PRs #93/#94). Bash arg parser in `ros1_server.sh` / `ros2_server.sh` gains a `--rate <Hz>` flag that forwards to the Python publisher; existing `MESSAGE` env var override unchanged. Counter (`#1`, `#2`, ...) is preserved -- now incremented inside the Python process so each message is still distinguishable on the wire. Test churn: removed 4 old regression guards (counter-via-bash-loop, log-before-publish ordering -- both implementation details of the now-removed bash pattern), added 8 new guards covering the Python helpers (exists / executable / `--help` / handoff / `--rate` parsing). Total bats `56 -> 60`; `ros_env.bats` `31 -> 35`; Demo helpers section `14 -> 18`.

- **IMAGE_NAME pinned to `ros1_bridge`** via `config/docker/setup.conf` `[image] rule_1 = string:ros1_bridge`. Previously the rule chain `prefix:docker_, suffix:_ws, @basename` derived the image tag from the checkout directory, so worktrees (`ros1_bridge-rate`) and `/tmp` test clones (`ros1_bridge_test/ros1_bridge`) produced separate image tags (`ros1_bridge-rate:devel`, etc.) bloating `docker image ls` and breaking muscle-memory for `docker exec ros1_bridge ...`. The `string:` rule type always matches first, so all checkouts now produce identical `ros1_bridge:{devel,runtime,test}` tags. The remaining `prefix:` / `suffix:` / `@basename` rules are kept in setup.conf as inactive documentation -- they're unreachable now but explain the historical detection chain.
...
- 4-language READMEs' Build section now uses the `-t` / `--target` flag form (`./build.sh -t test`, `./build.sh -t runtime`) for parity with `./run.sh -t <stage>` and `./exec.sh -t <stage>` (closes [#86](https://github.com/ycpss91255-docker/ros1_bridge/issues/86)). `build.sh -t / --target` shipped in base v0.26.0 ([base#280](https://github.com/ycpss91255-docker/base/issues/280)) as a backward-compatible alias for the positional TARGET; the positional form still works. The decision to prefer the flag form in downstream READMEs was locked in base v0.27.0's wrapper UX cheat sheet ([base#291](https://github.com/ycpss91255-docker/base/issues/291)). Doc-only.

- 4-language READMEs no longer document direct `docker build` / `docker compose build --build-arg` as escape hatches. Two sections collapsed: "Switch ROS 2 distro" loses Option 2 (`docker build --build-arg ROS2_DISTRO=jazzy`); "Bridge Configuration" loses the "override at build time without changing the symlink" `docker compose build --build-arg BRIDGE_FILE=...` paragraph. Both bypassed `setup.sh`'s drift detection: a direct `docker build` produces an image whose contents disagree with `.env` / `compose.yaml` from the last `./build.sh` run, and the next regular `./build.sh` invocation silently overwrites the override -- the user has no signal that the previous one-off ever happened. The wrapper-side `--build-arg` pass-through tracked at [base#279](https://github.com/ycpss91255-docker/base/issues/279) is being closed as won't-fix on the same rationale: the intended pattern is `./setup.sh set build.arg_N KEY=VALUE` + `./build.sh` (persistent, drift-tracked), never a bypass. Doc-only.

- 4-language READMEs' "Build" / "Run" / "Bridge Configuration" / "Switch ROS 2 distro" sections now prefer wrapper scripts (`./build.sh runtime`, `./run.sh -t runtime`) over the previous direct `docker build` / `docker compose build` / `docker run` examples. The runtime stage gained an auto-emitted compose service in template's recent versions (`#215` Dockerfile stage auto-emit), so the stale "tracked upstream in template" comment is removed -- `./run.sh -t runtime` is now the canonical way to launch the auto-bridge image. Two remaining one-off `--build-arg` escape hatches (Switch ROS 2 distro Option 2; Bridge config override) keep their direct `docker build` / `docker compose build` examples but each carries a "pending [base#279](https://github.com/ycpss91255-docker/base/issues/279)" note; once `./build.sh --build-arg KEY=VALUE` lands those sections will collapse to the wrapper form. Doc-only.

- **All 7 repo-local shell scripts now use `set -euo pipefail`** (closes [#81](https://github.com/ycpss91255-docker/ros1_bridge/issues/81)). Previously the 6 runtime helpers (`script/{entrypoint,ros_entrypoint,ros1_server,ros1_client,ros2_server,ros2_client}.sh`) ran on `set -e` only -- `-u` was deliberately omitted because ROS's setup.bash chain (catkin profile.d + ament prefix setup) dereferences unbound vars (`ROS_MASTER_URI` in `10.roslaunch.sh`, `AMENT_TRACE_SETUP_FILES` in `setup.bash` line 8, plus more in the ament setup chain) that ROS upstream itself populates as `[ -n "$VAR" ]` existence checks. Each script now wraps its `source /opt/ros/*/setup.bash` calls with `set +u; ... ; set -u` brackets -- the canonical pattern for sourcing third-party setup scripts under strict mode (compare `venv` / `conda` / `nvm` / `pyenv` activation snippets). This isolates ROS upstream's `-u` quirks at the language boundary while keeping all our own logic under full strict-mode protection. `script/docker/colcon_build_bridge.sh` (added in #80) already used this pattern; the 6 runtime scripts are now consistent with it.

- **`MAKEFLAGS=-j` for `colcon build ros1_bridge` is now auto-detected** from `min(nproc, mem_gb / 2)` instead of being hardcoded to `-j2` (closes [#79](https://github.com/ycpss91255-docker/ros1_bridge/issues/79)). ros1_bridge's colcon build peaks ~1.5-2 GB per parallel job from message-type template instantiation; pure `-j$(nproc)` OOMs on Jetson hardware while `-j2` left significant CPU idle on standard GHA / dev machines. The heuristic adapts: GHA / dev x86_64 boxes (4-16c, 16-32 GB) pick `-j4` to `-j16`, Jetson Nano (4c / 4 GB) still picks `-j2`. Override via `--build-arg BUILD_JOBS=N` when the heuristic mis-estimates. Builder-stage helper script lives at `script/docker/colcon_build_bridge.sh` (Dockerfile COPYs to `/tmp/colcon_build_bridge.sh`); `--print-jobs` flag short-circuits the colcon step so smoke tests can verify the heuristic without re-building. Expected GHA CI wall-time reduction: ~16 min/shard -> ~11 min/shard.
- New convention: `script/docker/` for Dockerfile-internal helper scripts (build-time logic). `script/` root remains for runtime / container helpers (entrypoint, demo scripts). Aligns with template's `template/script/docker/` for wrapper symlinks. Dockerfile's lint stage now covers both `script/*.sh` and `script/docker/*.sh` via ShellCheck.
- 4-language READMEs' "Switch ROS 2 distro" section rewritten to recommend `./setup.sh set build.arg_4 ROS2_DISTRO=jazzy` (with `./setup_tui.sh` as the interactive alternative) instead of the previous "edit `setup.conf` manually" wording. The CLI path has been available in template since the `setup.sh set / show / list / add / remove` subcommand set landed; the README just hadn't caught up. setup.conf is still the source of truth -- this is purely a UX swap from "vim setup.conf" to a validated one-line CLI call.
- **BREAKING — bridge YAML configs relocated under `config/ros1_bridge/`** (closes [#70](https://github.com/ycpss91255-docker/ros1_bridge/issues/70)). The five bridge configs `config/{scan_bridge,release_bridge,demo_bridge,demo_services_1to2,demo_services_2to1}.yaml` moved to `config/ros1_bridge/<same-name>.yaml`. Motivation: template v0.22.0 introduced a layered `<repo>/config/` overlay convention (template#254) where `<repo>/config/` is expected to mirror the `template/config/` subdirectory structure (`shell/`, `pip/`, etc.). The previous flat `config/*.yaml` layout collided with that convention — bridge YAMLs would have been transiently misclassified as overlay files. Moving them into a `config/ros1_bridge/` namespace keeps `<repo>/config/` free for future template-style overlays while preserving the existing bridge-config UX.
  - **Migration**: users with a `bridge.yaml` symlink need to re-create it pointing at the new path, e.g. `ln -sf config/ros1_bridge/scan_bridge.yaml bridge.yaml`. The Dockerfile-internal fallback (closes #65) still kicks in for absent / broken symlinks and now points at `config/ros1_bridge/demo_bridge.yaml`. Explicit `--build-arg BRIDGE_FILE=...` invocations need the new path too.
  - 4-language READMEs updated end-to-end: TL;DR + Quick Start `ln -sf` snippets, "Bridge Configuration" table file paths, `--build-arg BRIDGE_FILE=...` example.
- **BREAKING — demo helper scripts moved to `/root/demo/`, container WORKDIR set to `/root/demo`** (also closes [#70](https://github.com/ycpss91255-docker/ros1_bridge/issues/70)). Previously `COPY --chmod=0755 script/ /` dumped both entrypoints AND demo helpers (`ros{1,2}_{server,client}.sh`) into `/`, so a user who `./exec.sh`'d in landed in `/bridge_ws` (build-time WORKDIR carryover) and `ls` showed nothing related to the demos. Now:
  - `script/entrypoint.sh` + `script/ros_entrypoint.sh` stay at `/` (referenced by `ENTRYPOINT` directives).
  - `script/ros{1,2}_{server,client}.sh` move to `/root/demo/`.
  - `WORKDIR /root/demo` set in devel + runtime stages, so `./exec.sh` lands the user directly inside the demo folder. `ls` at landing shows the 4 demo scripts.
  - Demo invocation examples in 4 READMEs updated: `./exec.sh /ros1_server.sh` -> `./exec.sh /root/demo/ros1_server.sh`. No `PATH` modification in this PR — demo invocation uses absolute paths; a future PR can layer `~/demo/` onto `PATH` via the template#254 `bashrc.d` drop-in mechanism if needed.
  - Existing smoke tests adjusted to assert against new paths; new `WORKDIR lands in /root/demo (closes #70)` regression test added asserting `pwd == /root/demo` at smoke stage. Total bats count `49 -> 50`.
